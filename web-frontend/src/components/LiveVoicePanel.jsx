/**
 * LiveVoicePanel — Full-screen LiveKit voice overlay for Voice Ledger.
 *
 * Design follows LIVEKIT_VOICE_UI_DESIGN_GUIDE.md:
 *  - Dark immersive overlay with animated orb
 *  - State-driven color palette (green/purple/cyan/amber/red)
 *  - Live transcript panel
 *  - Action cards via data channel
 *  - Session controls (mic toggle, end call)
 */
...
export default function LiveVoicePanel({ isOpen, onClose }) {
  ...
  return (
    <SessionProvider session={session}>
      <VoicePanelOverlay session={session} onClose={onClose} userName={user?.full_name || 'Guest'} />
    </SessionProvider>
  )
}
...
/* ================================================================
   The overlay — orb, transcript, action cards, controls
   ================================================================ */
...
function VoicePanelOverlay({ session, onClose, userName }) {
  ...
  // Action cards from data channel
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center"
         style={{ background: 'rgb(3 7 18 / 0.95)', backdropFilter: 'blur(24px)' }}>
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-white/70 tracking-wide">Voice Ledger Voice</span>
        </div>
      ...
      <div className="mt-6 flex flex-col items-center gap-1.5">
        <div className="flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isPulsing ? 'animate-pulse-dot' : ''}`}
        ...
        {isStarted && (
          <span className="text-[11px] text-white/25 font-mono">{userName} • live session</span>
        )}
      </div>
          ...
          {showTranscript && (
            <div ref={transcriptRef}
              ...
              {transcriptions.map((seg, i) => (
                <p key={seg.id || i} className="text-xs leading-relaxed text-emerald-300/70">
                  <span className="font-mono text-[9px] text-white/20 mr-1.5">AI</span>
                  {seg.text}
                </p>
              ))}
            </div>
        ...
        {!isStarted ? (
          ...
          <>
            ...
            <button onClick={handleEnd}
                    className="w-14 h-14 rounded-full flex items-center justify-center bg-red-500/15 text-red-400 hover:bg-red-500/25 hover:scale-105 active:scale-95 transition-all">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor">
                <path d="M3.12 7.56A9.93 9.93 0 0110 5a9.93 9.93 0 016.88 2.56l.83-.83A11.43 11.43 0 0010 4c-2.8 0-5.38 1-7.71 2.73l.83.83zM6 10.5c0-.28.22-.5.5-.5h7c.28 0 .5.22.5.5v3c0 .28-.22.5-.5.5h-7a.5.5 0 01-.5-.5v-3z" />
              </svg>
            </button>
          </>
        )}
      ...
      {/* ── Footer ── */}
      <div className="absolute bottom-4 text-[9px] text-white/15 font-mono tracking-wider">
        Powered by LiveKit • Deepgram • OpenAI
      </div>
    </div>
  )
}
  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 800 800" preserveAspectRatio="xMidYMid slice">
      <defs>
        ...
        {/* State-driven radial glow */}
        <radialGradient id="vp-glow-primary" cx="50%" cy="45%" r="35%">
          <stop offset="0%" stopColor={colors.ring} stopOpacity="0.12" />
          <stop offset="100%" stopColor={colors.ring} stopOpacity="0" />
        </radialGradient>
      ...
      <circle cx="400" cy="360" r="220" fill="none" stroke="#10B981" strokeWidth="0.3" strokeDasharray="4 12" opacity="0.04">
        <animateTransform attributeName="transform" type="rotate" from="360 400 360" to="0 400 360" dur="45s" repeatCount="indefinite" />
      </circle>
      <circle cx="400" cy="360" r="260" fill="none" stroke="#10B981" strokeWidth="0.2" strokeDasharray="2 18" opacity="0.03">
      ...
      <path d="M800 0 L740 0 L800 60Z" fill="#10B981" opacity="0.03" />
      <path d="M0 800 L60 800 L0 740Z" fill="#10B981" opacity="0.03" />
      ...
      <circle cx="680" cy="150" r="1" fill="#06B6D4" opacity="0.12">
        <animate attributeName="cy" values="150;135;150" dur="5.5s" repeatCount="indefinite" />
        <animate attributeName="opacity" values="0.12;0.25;0.12" dur="5.5s" repeatCount="indefinite" />
      </circle>
      <circle cx="200" cy="600" r="0.8" fill="#10B981" opacity="0.1">
        <animate attributeName="cy" values="600;585;600" dur="3.5s" repeatCount="indefinite" />
      </circle>
      <circle cx="650" cy="550" r="1.2" fill="#8B5CF6" opacity="0.08">
        <animate attributeName="cy" values="550;535;550" dur="6s" repeatCount="indefinite" />
      </circle>
